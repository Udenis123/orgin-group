...
export class SessionService implements OnDestroy {
  ...
  resetExpirationTimer(): void {
    ...
    this.cookieService.set('expirationTime', expirationTime.toString());
  ...
  clearSession(): void {
    ...
    if (this.isBrowser) {
      localStorage.setItem('redirectUrl', window.location.pathname); // Store current URL
      window.location.href = '/auth/0/1/0/login';
    }
  }
